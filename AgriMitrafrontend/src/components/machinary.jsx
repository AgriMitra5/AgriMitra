...
const Bikes=()=>{
    ...
    const handleSubmit=e=>{
        e.preventDefault()
        axios.post('http://localhost:8080/api/product',bike)
        .then(resp=>{
            toast.success(resp)
            setBike({
              "id":"",
    ...
    const handleFilter=id=>{
      if(id==0){
        ...
        axios.get('http://localhost:8080/api/product/filter/'+id)
        .then(resp=>{
          setData(resp.data)
        })
      }
    ...
    const handleDelete=(id)=>{
        const resp=window.confirm('Are you sure you want to delete this product ?')
        if(resp){
          ...
          .catch(error=>{
            toast.error('Product already used in bookings')
          })
        }
    }
  ...
  <div className="col-sm-8">
    ...
    <h5 className="p-2 mb-3" style={{borderBottom: '2px solid green'}}>Product</h5>
